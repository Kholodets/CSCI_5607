...
void write_ppm(char* imgName, int width, int height, int bits, const uint8_t *data){
	...
	for (int i = 0; i < height; i++){
		for (int j = 0; j < width; j++){
			int shift = 8-bits;
			r = data[i*width*4 + j*4 + 0] >> shift;  //Red
			g = data[i*width*4 + j*4 + 1] >> shift;  //Green
			b = data[i*width*4 + j*4 + 2] >> shift;  //Blue
			a = data[i*width*4 + j*4 + 3] >> shift;  //Alpha
			ppmFile << r << " " << g << " "  << b << " " ;
		}
	}
...
uint8_t* read_ppm(char* imgName, int& width, int& height){
	...
	if (popcount(maximum + 1) != 1 || maximum > 255) {
	...
	int r, g, b;
	int shift = 8-popcount(maximum);
	for (int i = 0; i < height; i++){
		for (int j = 0; j < width; j++){
			ppmFile >> r >> g >> b;
			//the additional sum here puts it in the "middle of the bucket"
			img_data[i*width*4 + j*4] = (r << shift) + (128 >> (8 - shift));  //Red
			img_data[i*width*4 + j*4 + 1] = (g << shift)+ (128 >> (8 - shift));  //Green
			img_data[i*width*4 + j*4 + 2] = (b << shift)+ (128 >> (8 - shift));  //Blue
...
Image::~Image (){
	delete[] data.raw;
	//changed from delete data.raw;
	//please give us correct code if you want us to compile with -fsanitize=address.... :p
