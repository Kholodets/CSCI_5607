...
unsigned char* loadImage(int& img_w, int& img_h){
	...
	for (int i = 0; i < img_h; i++){
		...
		for (int j = 0; j < img_w; j++){
			...
			img_data[i*img_w*4 + j*4 + 1] = igam(green);
...
float square_edge_dist(Point2D p, Point2D t1, Point2D t2, Point2D t3, Point2D t4){
	MultiVector s1 = MultiVector(t1).vee(t2);
	MultiVector s2 = MultiVector(t2).vee(t3);
	MultiVector s3 = MultiVector(t3).vee(t4);
	MultiVector s4 = MultiVector(t4).vee(t1);
	...
	if (s3p.vee(t3).vee(p).s * s3p.vee(t4).vee(p).s < 0 && (d = MultiVector(p).normalized().vee(s3p.normalized()).magnitude()) < min) {
		min = d;
	}

	MultiVector s4p = s4.dot(MultiVector(p)) * s4;
	if (s4p.vee(t4).vee(p).s * s4p.vee(t1).vee(p).s < 0 && (d = MultiVector(p).normalized().vee(s4p.normalized()).magnitude()) < min) {
		min = d;
	}
	return min;
}

float square_vertex_dist(Point2D p, Point2D t1, Point2D t2, Point2D t3, Point2D t4){
	MultiVector s1 = MultiVector(t1).vee(t2);
	MultiVector s2 = MultiVector(t2).vee(t3);
	MultiVector s3 = MultiVector(t3).vee(t4);
	MultiVector s4 = MultiVector(t4).vee(t1);
	...
	if ((d = MultiVector(p).normalized().vee(t4).magnitude()) < min) {
		min = d;
	}
...
void mouseClicked(float m_x, float m_y){
	...
	bool sides[4];
	sides[0] = MultiVector(clicked_mouse).vee(l1).s > 0;
	sides[1] = MultiVector(clicked_mouse).vee(l2).s > 0;	
	sides[2] = MultiVector(clicked_mouse).vee(l3).s > 0;
	sides[3] = MultiVector(clicked_mouse).vee(l4).s > 0;
	

	float edist = square_edge_dist(clicked_mouse, p1, p2, p3, p4);
	float vdist = square_vertex_dist(clicked_mouse, p1, p2, p3, p4);



	if (((vdist < edist) && (vdist < 0.05)) || (vdist < 0.02)) {
		do_scale = true;
	} else if (edist < 0.03) {
		do_rotate = true;
...
void mouseDragged(float m_x, float m_y){
	...
	if (do_scale){
		...
		Line2D axis = Line2D(MultiVector(rect_pos).vee(MultiVector(clicked_mouse)));
		HomogeneousPoint2D proj = HomogeneousPoint2D(MultiVector(axis).dot(MultiVector(cur_mouse)).times(MultiVector(axis)));
		float scalef = MultiVector(proj).normalized().vee(MultiVector(rect_pos).normalized()).magnitude() /
			MultiVector(clicked_mouse).normalized().vee(MultiVector(rect_pos).normalized()).magnitude();
		rect_scale = clicked_size * scalef;
	}

	if (do_rotate){
		Line2D axis = Line2D(MultiVector(rect_pos).vee(MultiVector(clicked_mouse)));
		Line2D rot = Line2D(MultiVector(rect_pos).vee(MultiVector(cur_mouse)));

		float angle = acos(MultiVector(rot).normalized().dot(MultiVector(axis).normalized()).s);
		if (MultiVector(rect_pos).vee(MultiVector(clicked_mouse)).vee(MultiVector(cur_mouse)).s > 0) {
			angle =  - angle;
		}
